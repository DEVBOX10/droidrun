...
from droidrun.agent import DroidRunAgent
...
async def run_command(
    ...
    if provider.lower() == "openai":
        ...
        if not api_key:
            console.print(
                "[bold red]Error:[/] GEMINI_API_KEY environment variable not set"
            )
            return
        if not model:
    ...
    try:
        ...
        try:
            ...
            if success:
                ...
                console.print(
                    "[bold yellow]Execution completed but goal may not have been achieved.[/]"
                )
            ...
            if "does not support vision" in str(e):
                ...
                console.print(
                    "[yellow]Please specify a vision-capable model with the --model flag.[/]"
                )
...
@click.option(
    "--provider",
    "-p",
    help="LLM provider (openai, ollama, anthropic, gemini)",
    default="openai",
)
